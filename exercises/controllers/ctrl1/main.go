...
func main() {
	...
	go func() {
		...
		for i := range 3 {
			...
			// Get was called... and that's it. The queue still thinks this
			// worker is processing the item, so the re-added copy is held
			// back forever. Something is missing here.
		}
		processed <- got
	}()
