...
func main() {
	...
	// The List happens... and its resourceVersion — the exact point in
	// history where the watch should pick up — is thrown away.
	if _, err := cs.CoreV1().Pods(ns).List(ctx, metav1.ListOptions{}); err != nil {
		exkit.Failf("listing pods: %v", err)
	}

	// ResourceVersion "0" means "replay the server's cache first": old-1,
	// old-2 and old-3 all arrive as fresh Added events before new-pod does.
