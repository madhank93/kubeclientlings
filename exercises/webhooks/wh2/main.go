...
func mutate(review *admissionv1.AdmissionReview) *admissionv1.AdmissionResponse {
	...
	// The patch bytes are set, but PatchType is never assigned — so it stays
	// nil and the apiserver drops the patch silently. Set it to
	// admissionv1.PatchTypeJSONPatch (a *PatchType, so take its address).
