...
func main() {
	...
	exkit.WaitFor(ctx, "2 ready endpoints behind the service", func(ctx context.Context) (bool, error) {
		// "service-name=web" is a guess — and a wrong one. The real label is
		// a well-known constant in the discovery/v1 package. Selecting on a
		// label nothing carries returns an empty list forever.
		slices, err := cs.DiscoveryV1().EndpointSlices(ns).List(ctx, metav1.ListOptions{
			LabelSelector: "service-name=web",
		})
		if err != nil {
			return false, err
		}
	...
	exkit.Successf("found the service's EndpointSlices — 2 ready backends")
}
