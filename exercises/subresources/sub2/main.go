...
func main() {
	...
	// The status subresource is enabled, so the MAIN Update endpoint silently
	// ignores this status change — the phase stays empty. Write it through the
	// /status subresource with UpdateStatus instead.
	if _, err := dyn.Resource(gvr).Namespace(ns).Update(ctx, got, metav1.UpdateOptions{}); err != nil {
