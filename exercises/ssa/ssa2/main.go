...
func main() {
	...
	// mgr-b wants the same field mgr-a owns. Without Force this is a conflict
	// error. Set Force:true to knowingly take the field over.
	applyB := applyconfigcorev1.ConfigMap("shared", ns).WithData(map[string]string{"owner": "from-b"})
	if _, err := cs.CoreV1().ConfigMaps(ns).Apply(ctx, applyB, metav1.ApplyOptions{FieldManager: "mgr-b", Force: false}); err != nil {
