...
type WidgetSpec struct {
	// The unstructured object below has a "size" key — but this tag says
	// "sizes", so the converter finds no match and Size stays 0. The json tag
	// must equal the key exactly.
	Size int64 `json:"sizes"`
}
