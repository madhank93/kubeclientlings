...
func main() {
	...
	if _, err := cs.CoreV1().Pods(ns).Create(ctx, exkit.NginxPod(ns, "target"), metav1.CreateOptions{}); err != nil {
	...
	// The pod IS in the cache — but not under this key. Bare names aren't
	// how stores index namespaced objects. What does MetaNamespaceKeyFunc
	// build?
	key := "target"
