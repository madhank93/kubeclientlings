...
func main() {
	...
	err := informer.AddIndexers(cache.Indexers{
		"byTier": func(obj any) ([]string, error) {
			...
			// This files every pod under its NAME. The lookup below asks for
			// "frontend", which is nobody's name, so it comes back empty —
			// and empty is not an error, which is what makes this quiet.
			// File them under the value the lookup actually uses.
