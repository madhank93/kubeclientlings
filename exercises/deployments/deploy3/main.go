...
func main() {
	...
	// Which container should get the new image? The patch never says.
	// Strategic merge patch identifies list elements by their merge key —
	// for containers, that's "name".
	patch := []byte(`{"spec":{"template":{"spec":{"containers":[{"image":"` + newImage + `"}]}}}}`)
