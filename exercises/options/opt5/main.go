...
func main() {
	...
	// This asks for ONE page of 5 and stops. The response carries a
	// metadata.continue token — the server's cursor — and dropping it means
	// the other 7 configmaps are never fetched. Wrap this in a loop that
	// feeds the previous response's Continue back in, and stops when the
	// returned token is empty.
